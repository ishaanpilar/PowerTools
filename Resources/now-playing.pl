...
my $symbol = DynaLoader::dl_find_symbol($handle, "powertools_now_playing_get")
    or die "now-playing: adapter entry point missing: " . DynaLoader::dl_error() . "\n";
DynaLoader::dl_install_xsub("main::powertools_now_playing_get", $symbol);
powertools_now_playing_get();
