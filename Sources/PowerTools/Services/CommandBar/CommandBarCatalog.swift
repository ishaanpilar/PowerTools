...
enum CommandBarCatalog {
    ...
    /// showing its own shortcut. This is how the bar reaches past PowerTools
    ...
    /// runs, so one row serves both "gh" and "gh powertools".
    ...
    private static func keepOnShelf(_ text: String) {
        let board = NSPasteboard(name: NSPasteboard.Name("com.powertools.commandbar.selection"))
        board.clearContents()
