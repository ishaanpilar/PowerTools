...
enum CommandBarSource: String, CaseIterable, Identifiable {
    /// What PowerTools itself can do. Always on: it is what the bar is for.
    case actions
    case apps
    case menus
    case windows
    case quitApps
    case settingsPages
    /// The Mac's own Settings panes, which are not PowerTools's and can be
...
enum CommandBarPreferences {
    ...
    ///
    /// What this Mac holds (the apps, PowerTools's own actions, the Settings
