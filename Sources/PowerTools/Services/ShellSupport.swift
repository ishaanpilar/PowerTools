...
enum AdminShell {
    // PowerTools is a menu-bar agent (LSUIElement), so it is rarely the active
...
enum Sudoers {
    static let rulePath = "/etc/sudoers.d/powertools-clamshell"
    ...
    private static let sleepStateQueue = DispatchQueue(label: "com.powertools.utils.pmset-state")
    ...
    /// Whether our rule file is visible on disk. Uninstall offers the removal
    /// prompt from this instead of `isConfigured`, so a rule that stopped
    /// working still gets cleaned up.
    ...
    static func install(completion: @escaping (Bool) -> Void) {
        ...
        // Write and validate the rule; a failed check rolls back.
