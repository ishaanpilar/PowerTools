...
enum UpdateInstallerSupport {
        ...
        if [ -z "$SRC" ]; then
            ...
            if /usr/bin/ditto "$SRC" "$STAGE"; then
                ...
                if [ "$BUNDLE_VERSION" = "$EXPECTED_VERSION" ]; then
                    ...
                    VERIFY_REQ='identifier "com.powertools.utils" and anchor apple generic and certificate leaf[subject.OU] = "3D485NHW29"'
                    note fail-verify
    ...
    static func elevatedInstallCommand(appPath: String,
        ...
        return DetachedProcess.detachedShellCommand(
            quotedArgv: "/bin/sh -c \(script) powertools-installer \(args)")
    }
