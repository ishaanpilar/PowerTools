...
enum UpdateShowcaseInfo {
    // A one-shot, per-release mechanism: the intro only ever renders when the
    // running version equals `releaseVersion` exactly, so these three constants
    // are inert until PowerTools ships a showcase of its own. Replace all three
    // together (version, asset name, checksum) when it does — the asset here is
    // still the inherited upstream clip and shows upstream's UI.
    static let releaseVersion = "3.1.4"
    static let mediaAssetName = "upstream-3.1.4-showcase-1.mp4"
    static let mediaSHA256 = "88031b2b48708b8eb96248fef1143432a0600382b59ad5ea39e0746af27ab9e8"

    static var remoteMediaURL: URL {
        URL(string: "https://github.com/\(AppInfo.repositorySlug)/releases/download/v\(releaseVersion)/\(mediaAssetName)")!
    }
    ...
    static var cacheDirectory: URL {
        ...
        let bundleID = Bundle.main.bundleIdentifier ?? "com.powertools.utils"
        return base
