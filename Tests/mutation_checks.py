...
MUTATIONS = [
    ...
    ("validator accepts an invented app name", "health-coach",
    ...
    ("validator accepts an invented percentage", "health-coach",
     "Sources/PowerTools/Services/HealthCoach/HealthNarratorValidator.swift",
    ...
    ("validator accepts all-is-well during a critical finding", "health-coach",
     "Sources/PowerTools/Services/HealthCoach/HealthNarratorValidator.swift",
    ...
    ("app-name check rejoins across a bullet boundary", "health-coach",
     "Sources/PowerTools/Services/HealthCoach/HealthNarratorValidator.swift",
     "\"\\\\b[A-Z][A-Za-z0-9]*(?:[ ][A-Z][A-Za-z0-9]*)*\\\\b\"",
     "\"\\\\b[A-Z][A-Za-z0-9]*(?:\\\\s[A-Z][A-Za-z0-9]*)*\\\\b\"",
     "a reply that only cites evidence the finding actually carries is accepted"),
    ("D4 boundary check no longer reads the real provider", "health-coach",
     "Sources/PowerTools/Services/HealthCoach/HealthNarratorService.swift",
     "settings: settings, system: system, providerBoundary: option.boundary)",
     "settings: settings, system: system, providerBoundary: .local)",
    ...
    ("a real model call is never recorded in the usage ledger", "health-coach",
     "Sources/PowerTools/Services/HealthCoach/HealthNarratorService.swift",
    ...
    ("redaction applies to an on-device request too", "health-coach",
     "Sources/PowerTools/Services/HealthCoach/HealthNarratorService.swift",
    ...
    ("redaction ignores the Settings toggle", "health-coach",
     "Sources/PowerTools/Services/HealthCoach/HealthNarratorService.swift",
    ...
    ("validator's evidence names ignore the redaction resolver", "health-coach",
     "Sources/PowerTools/Services/HealthCoach/HealthNarratorValidator.swift",
     "func addApp(_ name: String) { names.insert(nameForApp(name).lowercased()) }",
     "func addApp(_ name: String) { names.insert(HealthNarratorPrompt.sanitizeName(name).lowercased()) }",
     "a reply that names the real app anyway is rejected as invented, since the model was never shown that name"),
    ("app category matched by substring", "health-coach",
    ...
    ("explain with nothing wrong still calls the model", "health-coach",
     "Sources/PowerTools/Services/HealthCoach/HealthCoachTriggerPolicy.swift",
     "guard trigger != .explainPressed || !findings.isEmpty else { return .useTemplate(.nothingToExplain) }",
     "guard true else { return .useTemplate(.nothingToExplain) }",
     "pressing Explain with nothing wrong never spends a capped model call"),
    ("a rejected reply skips the usage caps", "health-coach",
     "Sources/PowerTools/Services/HealthCoach/HealthNarratorService.swift",
