...
Reuse the global shortcut value type with an explicit modifier opt-out for editor bindings. Global shortcuts retain their existing validation. Recording also carries the original event flags so lock state is not lost. Before saving, reject editor commands, other tool bindings, enabled PowerTools shortcuts, both window layout shortcut modes and macOS shortcuts, keeping the previous binding. System comparisons preserve the Fn requirement; unsupported Fn-letter combinations cannot be silently saved as plain letters. Reserved and malformed bindings are filtered independently of keyboard layout when reading preferences.
